...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['request_computer'])) {
    // Check again to avoid duplicate requests
    ...
    if ($existing->num_rows > 0) {
        ...
        // Insert into comp_request
        $stmt = $conn->prepare("INSERT INTO comp_request (student_number, date_requested, status) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $student_number, $date_requested, $status);

        if ($stmt->execute()) {
            // Log into comp_logs
            $log_stmt = $conn->prepare("INSERT INTO comp_logs (student_number, status) VALUES (?, ?)");
            $log_stmt->bind_param("ss", $student_number, $status);
            $log_stmt->execute();
            $log_stmt->close();
